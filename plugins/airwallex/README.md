...
Teaches Claude and Cursor how to use Airwallex — Billing, Payouts, Issuing, and Treasury — via either the `airwallex` CLI or an Airwallex MCP server.

The same skills work on both surfaces. Each `SKILL.md` is surface-neutral — workflow logic is shared, and the agent picks up the surface-specific details (command names, tool names, auth, pagination) from `awx-best-practices/references/surface-quickstart.md` and from the connected toolchain itself.
...
- **MCP:** An Airwallex MCP server connected to your Claude Code or Cursor client. Configure the MCP server entry per your host's MCP setup docs.
